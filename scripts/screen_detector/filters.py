# Modified from https://github.com/tensorflow/addons/blob/v0.15.0/tensorflow_addons/image/filters.py#L218-L302
    ...
    with tf.name_scope(name or "gaussian_filter2d"):
        ...
        if isinstance(filter_shape, (list, tuple)):
            if len(filter_shape) != 2:
                raise ValueError("filter_shape should be a float or a tuple/list of 2 floats")
        else:
            filter_shape = (filter_shape,) * 2

        image = tf.convert_to_tensor(image, name="image")
        sigma = tf.convert_to_tensor(sigma, name="sigma")
        filter_shape = tf.convert_to_tensor(filter_shape, name="filter_shape")

        # is_less = tf.reduce_any(tf.less(sigma, tf.constant(0)))
